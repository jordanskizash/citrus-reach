...
export const Publish = ({ initialData, customDomain, isCustomDomainVerified }: PublishProps) => {
  ...
  const getPublishUrl = () => {
    if (customDomain && isCustomDomainVerified) {
      return `https://${customDomain}/blog/${initialData.slug ?? initialData._id}`
    }
    return `${origin}/blog/${initialData.slug ?? initialData._id}`
  }
      ...
      <PopoverContent className="w-72" align="end" alignOffset={8} forceMount>
        {initialData.isPublished ? (
          <div className="space-y-4">
            ...
            {!customDomain && (
              <div>
                <Link href="/settings" className="text-xs text-sky-500 hover:text-sky-700 hover:underline">
